...
const executionWorkspaces = new Map<string, any>();
...
export async function executeCommands(
    ...
    for (const command of commands) {
        ...
        if (command.type.startsWith('api.')) {
            ...
            const args = command.args.map((arg: any) => {
            ...
            for (let i = 0; i < times; i++) {
                ...
                // --- START of change for live editing ---
                const jsGenerator = Blockly.JavaScript;
                
                // For live editing, we must read the block structure from the workspace the user is editing,
                // which is the main workspace. This only applies to the currently active sprite.
                // For other sprites running in the background, they continue with their snapshot.
                const sourceWorkspace = (sprite.id === appState.activeSpriteId) 
                    ? Blockly.getMainWorkspace()
                    : executionWorkspace;
                
                const loopBlock = sourceWorkspace.getBlockById(command.blockId);
                
                // If the block was deleted mid-run, exit the loop.
                if (!loopBlock) break;

                // Re-generate the command list for the blocks inside the loop on every iteration.
                const substackCode = jsGenerator.statementToCode(loopBlock, 'SUBSTACK');
                
                if (substackCode) {
                    try {
                        const liveSubstackCommands = JSON.parse(`[${substackCode}]`);
                        // IMPORTANT: Execute the newly compiled commands within the original, isolated execution context.
                        await executeCommands(liveSubstackCommands, sprite, executionWorkspace, options, true);
                    } catch (e) { 
                        // This can happen if the substack is empty, which is not an error.
                    }
                }
                // --- END of change for live editing ---

                if (isForever) await new Promise(resolve => setTimeout(resolve, 0)); // Yield to prevent browser freezing
            }
        }
    }
}
 ...
 */
function getOrCreateExecutionWorkspace(sprite: Sprite): any | null {
    ...
    try {
        ...
        console.error(`Error loading workspace for sprite "${sprite.name}". Workspace state may be corrupt.`, e);
        newWorkspace.dispose(); // Clean up the failed workspace
...
// Global flag to prevent concurrent execution cycles.
let isExecutionCycleActive = false;

/**
 * Starts the execution of all "when green flag clicked" scripts.
 * This function prepares the environment and runs the scripts to completion.
 */
async function startGreenFlagScripts() {
    // 1. Set global running state
    ...
    // 2. Persist the current state of the main editor to the active sprite's data
    const workspace = Blockly.getMainWorkspace();
    if (appState.activeSpriteId && appState.sprites[appState.activeSpriteId]) {
        try {
            appState.sprites[appState.activeSpriteId].workspaceState = Blockly.serialization.workspaces.save(workspace);
        } catch (e) {
            console.error("Critical error: Failed to save main workspace. Aborting execution.", e);
            return; // Abort if saving fails
        }
    }
    
    // 3. Collect all script execution promises
    ...
    for (const sprite of Object.values(appState.sprites)) {
        ...
        startBlocks.forEach((block: any) => {
            const generatedCode = jsGenerator.blockToCode(block);
            if (generatedCode) {
                try {
                    const commands = JSON.parse(`[${generatedCode}]`);
                    allScriptPromises.push(executeCommands(commands, sprite, execWorkspace));
                } catch (e) { /* ignore empty or malformed scripts */ }
            }
        });
    }

    // 4. If no scripts exist, we don't need to wait for anything.
    if (allScriptPromises.length === 0) {
        return;
    }

    // 5. Await all scripts to complete.
    await Promise.all(allScriptPromises);
}

/**
 * The main entry point for the green flag click.
 * It ensures that any previous run is stopped before starting a new one.
 */
export async function execute() {
    if (isExecutionCycleActive) {
        return; // Prevent re-entrant calls
    }
    isExecutionCycleActive = true;

    try {
        // 1. Stop everything that might be running.
        stopAllExecutions();

        // 2. Wait for a very short period to allow any pending async cleanup to finish.
        await new Promise(resolve => setTimeout(resolve, 10));

        // 3. Start the new execution cycle.
        await startGreenFlagScripts();

    } catch (err) {
        console.error("An unexpected error occurred during the execution cycle:", err);
    } finally {
        // 4. If execution finished naturally (i.e., not stopped by the user via the stop button),
        // we call stopAllExecutions() again to clean up and reset the UI to the idle state.
        if (!appState.stopSignal) {
            stopAllExecutions();
        }
        
        // 5. Release the lock to allow another execution.
        isExecutionCycleActive = false;
    }
}
